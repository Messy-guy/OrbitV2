...
- **Architectural Decision (DEC-003)**: Dual-layer Provenance Reconciliation for Verified File Changes and Completed Work. `ProjectHealthSnapshot` reconciles ephemeral working-tree modifications (validated against filesystem presence to grant `git_verified`) with persistent `EventStore` verification records (`file_verified` / `behavior_verified`), rejecting unverified phantom files while eliminating blindspots for uncommitted edits.
- **Architectural Decision (DEC-004)**: Profile-aware Home Normalization for Canonical Orbit Storage. When agents run within isolated profiles (`~/.orbit/profiles/<profile>`), `getOrbitHomeDir()` normalizes to the canonical host user home directory (`~`) and respects `ORBIT_HOME`, ensuring `EventStore` and `HandoffService` access the authoritative persistent ledger (`~/.orbit/projects/<slug>/events.jsonl`) rather than looking inside an isolated profile subdirectory.
- **Architectural Invariant (INV-004)**: Canonical EventStore Single-Ledger Invariant. All project event ledgers, persistent memories, and handoffs must reside exclusively in the canonical host `~/.orbit/projects/<slug>/` storage hierarchy, regardless of child agent profile isolation or modified environment variables.
- **Error/Debugging Diagnosis**:
  1. An initial implementation attempted to extract architectural decisions and unresolved issues from the legacy `readProjectMemory` helper (`cwd/.orbit/DECISIONS.md`), which produced empty collections because Orbit's canonical state lives in the event ledger (`EventStore`). The implementation was corrected to replay from `EventStore` via `EventReplayEngine`, verifying 100% data recovery.
  2. `EventReplayEngine` lacked handling for `agent.activity` and behavior-verified task completions in `EventReplayEngine.replay()`, which caused `mission.progress.completed` to remain permanently empty during event replay. Corrected by adding typed activity replay and verification milestone promotion into `state.mission.progress.completed`.
  3. Profile-isolated `HOME` caused `EventStore.getEvents()` to search `~/.orbit/profiles/second-account/.orbit/projects/orbitv2/events.jsonl` where zero events existed. Diagnosed via `projectHealthSnapshot.test.ts` fallback behavior; resolved by updating `getOrbitHomeDir()` in `src/utils/pathUtils.ts` and synchronizing tests (`handoffBugFixesRegression.test.ts` and `productionHandoffPipeline.test.ts`) to use `EventStore.getProjectDir()`.
- **Rejected Approach**: Relying solely on Git commits for `completedWork`. During an active pre-handoff engineering session, commits are deferred (uncommitted working tree), which caused `completedWork` to be completely empty. The design now projects completed milestones from `EventStore` event streams (`agent.activity`, `verification.completed`, resolved issue milestones), retaining Git commits only as supplementary baseline history.
- **Unresolved Issue (ISSUE-042 / ISSUE-043)**: Monorepo multi-target sub-package health isolation. The root `ProjectHealthSnapshot` evaluates root-level manifests (`package.json`, `Cargo.toml`) and directories, but does not yet recursively evaluate independent sub-package git submodules or manifest health graphs in nested monorepo packages (e.g. `apps/mobile`). Safe to defer to future monorepo workspace enhancement.
