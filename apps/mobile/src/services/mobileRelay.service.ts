...
class MobileRelayService {
  ...
  async connect() {
    ...
    const relayUrl = process.env.EXPO_PUBLIC_API_URL || 'http://192.168.18.60:3000';
    console.log(`🔗 [Mobile Relay] Connecting to ${relayUrl}/relay with token length ${token.length}...`);
  ...
  sendAction(action: 'PAUSE' | 'STOP' | 'SEND_INPUT' | 'APPROVE', agentId?: string, projectId?: string, data?: any) {
